...
class Ball { 
  ...
  float radius = width / 27; 

  // Restrict the movement along the Y axis 
  float fix_Y = 0;

  Ball (float Y) { // Constructor. When the ball is created (Ball ball = new Ball()), this thing is called.
    ...
    velocity = new PVector(random(-1,4), 0, 10); 
  ...
  void update () { // Update. We Move the ball, check for collisions, and handle scoring in here.
    ...
    if (ballPos.z > 0) { // The player's wall is at z = 0.
      ...
      if ((intersect) || (first)) { // if the ball hits the paddle
        ...
        velocity.z *= -1; 
        // make sure the ball doesn't get stuck.
        ballPos.z = 0; 
